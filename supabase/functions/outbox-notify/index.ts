...
// 📤 Bandeja de salida (outbox), con el contenido del draft y un link directo
// a la ficha del contacto en el dashboard (viven.ch/dashboard/?lead=<id>) —
// ahí mismo se ve y se aprueba/descarta el borrador (todo el team lo ve).
//
// fix (2026-07-29, pedido de Sebastián): el link antes apuntaba directo a
// .../functions/v1/outbox-action (un endpoint de Supabase) — el gateway de
// Edge Functions fuerza Content-Type: text/plain + X-Content-Type-Options:
// nosniff en TODO lo que devuelven, así que el HTML de confirmación nunca se
// renderizaba: el browser mostraba el markup crudo como texto ("una página
// con código"). Se cambia a un deep-link al dashboard (requiere login, pero
// ahí sí renderiza bien) en vez de intentar un one-click sin login.
//
...
<div style="max-width:600px;margin:0 auto;padding:28px 16px">
  ...
  <div style="background:#ffffff;border-radius:0 0 14px 14px;padding:30px 26px">
    ...
    <div style="display:flex;gap:10px;flex-wrap:wrap">
      <a href="${leadUrl}" style="background:#0f1826;color:#ddf98f;text-decoration:none;font-weight:700;font-size:14px;padding:12px 20px;border-radius:100px;display:inline-block">👤 Ver y aprobar</a>
    </div>
    <p style="margin:20px 0 0;font-size:12.5px;color:#999">Te lleva directo a la ficha de ${esc(leadName)} — el borrador está ahí, con aprobar/descartar (necesita login).</p>
  </div>
</div></body>`;
