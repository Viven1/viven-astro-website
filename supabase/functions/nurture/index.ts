...
// LIFECYCLE AUTOMATION (estilo Keap, hecho en casa): secuencia automática de
// emails a leads NUEVOS, personalizada por idioma, fuente y servicio de interés.
//
...
// (el equipo ya lo está trabajando), máximo 1 email por paso (nurture_log).
// El toggle 🌱 del dashboard (app_settings.nurture) apaga los pasos 2-3;
// la bienvenida del paso 1 siempre sale (es transaccional y esperada).
//
...
function wrap(inner: string, unsub: string, lang: string): string {
...
Deno.serve(async (req) => {
  try {
    ...
    // toggle de los pasos 2-3 (app_settings.nurture = {"enabled": bool}); paso 1 siempre
    let steps23 = true;
    try {
    ...
    const now = Date.now();
    let out = { s1: 0, s2: 0, s3: 0, skipped: 0 };
    for (const r of leadsQ.data ?? []) {
      if (!r.email || TEST.test(r.email) || (r as { unsubscribed?: boolean }).unsubscribed) { out.skipped++; continue; }
      const age = now - new Date(r.created_at).getTime();
      let step = 0;
      if (age > 20 * 60e3 && age < 2 * 864e5 && !sent.has(r.id + ":1")) step = 1;
      else if (steps23 && age > 3 * 864e5 && age < 10 * 864e5 && isNuevo(r.status) && sent.has(r.id + ":1") && !sent.has(r.id + ":2")) step = 2;
      else if (steps23 && age > 7 * 864e5 && age < 21 * 864e5 && isNuevo(r.status) && sent.has(r.id + ":2") && !sent.has(r.id + ":3")) step = 3;
      if (!step) continue;
      if (body.dry_run) { out["s" + step as "s1"]++; continue; }
      const { subject, html } = mailFor(step, r as Record<string, unknown>);
      const unsub = `${SB_URL}/functions/v1/newsletter-unsub?l=${r.id}&t=${await unsubToken(r.id)}`;
