...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { topic = "", keyword = "", lang = "en", localize = false, source_title = "", source_lead = "", source_html = "", source_faq = [] } = await req.json();
...
Write a complete, genuinely useful article in ${language} about: "${t}".${localizeNote}${sourceNote}
